...
public class CustomerController {
	...
	public ResponseEntity<List<UserDepartmentResponse>> getAllUserDepartments(){
		...
		List<UserDepartmentResponse> customers = null;
		
		try {
			customers = customerService.getUserDepartMent();
			
		} catch (Exception ex) {
			ex.getMessage();
		}
		
		return new ResponseEntity<List<UserDepartmentResponse>>(customers, HttpStatus.OK);
	}
